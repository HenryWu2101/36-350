...
fun = density(x)
lines(fun, col="firebrick3",lwd=3)

integrate.xy(fun$x, fun$y, 0, 4)
```
...
hcubature(integrand, c(0, 0), c(pi/4, pi/2))$integral
```
...
# FILL ME IN
set.seed(100)
data3 = sample(1:100, 100000, replace = TRUE)
M3 = matrix(data3, ncol = 100)
uni = apply(M3, 1, function(x) {length(unique(x))})
hist(uni, main = "Histogram of unique int in each sample", 
     xlab = "unique int numbers")
mean(uni)
sd(uni)
```
...
my.fit.fun = function(my.par, my.data) {
  (my.data[2] - my.par[1]*
     (my.data[1]^2) - my.par[2]*my.data[2] - my.par[3])^2
}
my.fit.gradient = function(my.par, my.data) {
  c(sum(my.data^2), sum(my.data), 0)
}
my.par = c(0.3, -1.2, -3.5)
out = optim.out = suppressWarnings(
  optim(my.par,my.fit.fun,gr=my.fit.gradient,
        my.data=c(x,y),method="BFGS"))
f = function(x){
  out$par[1] * (x^2) + out$par[2] * x + out$par[3]
}
plot(x,y,type="l",ylab="f'(x)",lwd=3,col="peachpuff")
lines(x,f(x),type = "l", col="firebrick3",lwd=3)
```
...
matchDict = function(x) {
  ...
  res = apply(matrix(unlist(permn(v)), nrow = length(v)), 2, 
              function(x) paste(x, collapse = ""))
  res[res %in% GradyAugmented]
}
matchDict("rneup")
matchDict("srsets")
matchDict("LYPELU")
...
diff_ob = abs(mean(y[x==0]) - mean(y[x==1]))


B = 10000      # number of bootstrap replications

M_0 = apply(matrix(nrow = B, ncol = 10), 1, 
            function(x) {sample(1:20, 10, replace = FALSE)})

diff_v = apply(M_0, 2, function(x) abs((sum(y) - sum(y[x]))/10 - mean(y[x])))
(diff = length(diff_v[diff_v > diff_ob]))
(p = diff/B)
```
```
636 of the 10000 differences are greater than I observed.
Since the p-value = 0.064 > 0.05, we reject the null 
hypothesis. We have sufficient evidence that the actual
difference in population means is not zero
```
