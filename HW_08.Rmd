...
```{r linewidth=80}
readLines("https://raw.githubusercontent.com/HenryWu2101/36-350/main/README.md")
```
...
source_url("https://raw.githubusercontent.com/HenryWu2101/36-350/main/dark_and_stormy.R")
```
...
```{r linewidth=80}
source_url("https://raw.githubusercontent.com/HenryWu2101/36-350/main/dark_and_stormy.R")
source_url("https://raw.githubusercontent.com/HenryWu2101/36-350/new_branch/dark_and_stormy.R")
```
...
```{r linewidth=80}
source_url("https://raw.githubusercontent.com/HenryWu2101/36-350-Fork/main/check_mark.R")
```
---
...
testthat::test_that(desc = "Test that the lower and upper works", expect_silent(f("A")))
testthat::test_that(desc = "Test that the output is correct", expect_equal(f("a"), 0))
testthat::test_that(desc = "Test that non-char exception works", expect_error(f(12)))
testthat::test_that(desc = "Test that multi string warning works", expect_error(f("ab", "b")))
testthat::test_that(desc = "Test that multi string exception works", expect_output(f("ab")))
testthat::test_that(desc = "Test that multi string exception works", expect_equal(f("ab"), 0))
```
...
```{r linewidth=80}
f = function(x) {
  if (typeof(x) != "character") stop("input must be character")
  if (length(x) != 1) stop("the input must be of length 1")
  if (nchar(x) > 1) warning("char must be length 1")
  log(which(letters==x))
}
...
g = function(x) {
  tryCatch(f(x), 
    warning = function(c) {
      print("char must be length 1")
      log(which(letters==substr(x,1,1)))
    })
}

testthat::test_that(desc = "Test that the lower and upper works", expect_silent(g("A")))
testthat::test_that(desc = "Test that the output is correct", expect_equal(g("a"), 0))
testthat::test_that(desc = "Test that non-char exception works", expect_error(g(12)))
testthat::test_that(desc = "Test that multi string warning works", expect_error(g("a", "b")))
testthat::test_that(desc = "Test that multi string exception works", expect_output(g("ab")))
testthat::test_that(desc = "Test that multi string exception works", expect_equal(g("ab"), 0))
```
